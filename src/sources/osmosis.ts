...
export class OsmosisSource extends SwapSource {
  async realFetch(): Promise<void> {
    ...
    while (response.data.pagination.next_key != null) {
      response = await axios.get(
        this.endpoint + '/osmosis/gamm/v1beta1/pools?pagination.key=' + response.data.pagination.next_key,
      );
  ...
  async normalize(swaps) {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      const price = new BigNumber(filtered[i].poolAssets[1].token.amount)
        .dividedBy(new BigNumber(filtered[i].poolAssets[0].token.amount))
        .times(
          new BigNumber(filtered[i].poolAssets[0].weight).dividedBy(new BigNumber(filtered[i].poolAssets[1].weight)),
        );
      let traceA,
        traceB,
        denomA,
