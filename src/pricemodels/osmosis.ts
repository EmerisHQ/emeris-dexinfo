...
export function returnAmount(inputCoin: EmerisTransactions.AbstractAmount, pool: EmerisDEXInfo.Swap) {
  ...
  const effectivePrice = isReverse
    ? balanceB.plus(inputAmount.multipliedBy(2)).dividedBy(balanceA)
    : balanceA.plus(inputAmount.multipliedBy(2)).dividedBy(balanceB.multipliedBy(weightingFactor));
